...
1. linkedin_login(email,password) - Helps to log in to the linkedin page.Required attributes are two. Returns driver of
logged in page

2. fill_job_location(driver, position, location) - Helps to fill position, location and need driver as argument from
"linkedin_login(email,password)" and returns driver of filtered jobs and total pages present.

filter applied - Past week in Date posted, Entry level & Internship in experience level

3. collect_links(driver, total_pages) - Helps to collect the links present in all the pages. Returns links collected.
Required attributes are driver and total pages received from "fill_job_location(driver, position, location)"

4. job_dict_base() - Helps to hold all the job values. it has empty dictionary and returns the same

5. waitpoint() - Helps us to know the how many links scraped so far and returns wait_point

6. job_detailed_scraper(links, Job_Overall_Details, wait_point) - Helps in scrapping of provided job links and returns
the dictionary of scraped jobs. Needs 3 argument links,job_dict_base(), waitpoint()
...
def job_detailed_scraper(links, Job_Overall_Details, wait_point):  # giving links and overall details dict and wait point
